...
socket.onmessage = function(message) {
    ...
    // Updates the yellow text that tells whether or not the background process is running
        ...
        if (data["output"] == "start") {
            ...
            $("#estop").html("Estop")
            $("#estop").css({"background-color": "rgb(255, 0, 0)", "border": "none", "cursor": "pointer"});
        }
        else
        ...
        if (data["output"] == "estop") {
            ...
            $("#estop").css({"background-color": "rgb(0, 200, 0)", "border": "none", "cursor": "pointer"});
        }
        else {
            robot_is_estopped = false;
            $("#estop").html("Estop")
        ...
        for (var program in data["output"]) {
            $(".program-list").html($(".program-list").html() + "<br><button id='program' onclick=displayProgram('" + program + "')>" + program + "</button>");
        }
        getPrograms();
    }
